	...
	function calendarWidget(el, params) { 
		...
		var table = '';
            ...
            for (j=0;j<42;j++){
              ...
              if ((j<firstDay)){
                table += ('<td class="date-item" id="'+zeroPad(prev_m+1,2)+'-'+zeroPad((prev_days-firstDay+j+1),2)+'-'+prev_y+'"><span class="day">'+ (prev_days-firstDay+j+1) +'</span></td>');
			  } else if ((j>=firstDay+getDaysInMonth(month,year))) {
				i = i+1;
				table += ('<td class="date-item" id="'+zeroPad(next_m+1,2)+'-'+zeroPad(i,2)+'-'+next_y+'"><span class="day">'+ i +'</span></td>');			 
              }else{
